# ─────────────────────────────────────────────────────────────────────────────
# Greek Financial Regulation MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t greek-financial-regulation-mcp .
# Run:    docker run --rm -p 3000:3000 greek-financial-regulation-mcp
#
# The image bakes /app/data/hcmc.db at build time. Override HCMC_DB_PATH
...
ENV HCMC_DB_PATH=/app/data/hcmc.db
...
# Bake the pre-built database into the image so /app/data/hcmc.db resolves
# at runtime without a bind mount.
#
# The explicit `data/hcmc.db` reference is required — `.github/workflows/
...
# We then COPY it into the image at /app/data/hcmc.db (HCMC_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/hcmc.db
